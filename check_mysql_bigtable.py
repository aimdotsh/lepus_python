...
def check_mysql_bigtable(host,port,username,password,server_id,tags,bigtable_size):
    try:
        ...
        try:
           bigtable=curs.execute("SELECT table_schema as 'DB',table_name as 'TABLE',CONCAT(ROUND(( data_length + index_length ) / ( 1024 * 1024 ), 2), '') 'TOTAL' , table_comment as COMMENT FROM information_schema.TABLES where TABLE_SCHEMA not in ('information_schema','mysql','performance_schema','sys','test') and engine is not null ORDER BY data_length + index_length DESC limit 10;");
           #print bigtable_size
           if bigtable:
