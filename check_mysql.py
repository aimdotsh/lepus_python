...
def check_mysql(host,port,username,password,server_id,tags,replchannel):
    try:
        uuid_tag=uuid.uuid1()
        ...
        thread_cache_size = func.get_item(mysql_variables,'thread_cache_size')
        ############################# GET INNODB INFO ##################################################
        #innodb variables
        ...
        innodb_buffer_pool_wait_free = int(func.get_item(mysql_status,'Innodb_buffer_pool_wait_free'))
        ...
        innodb_row_lock_time = int(func.get_item(mysql_status,'Innodb_row_lock_time'))
        innodb_row_lock_waits = int(func.get_item(mysql_status,'Innodb_row_lock_waits'))
        innodb_log_waits = int(func.get_item(mysql_status,'Innodb_log_waits')) 
        ...
        opened_tables = func.get_item(mysql_status,'Opened_tables')
        ...
        max_used_connections =  func.get_item(mysql_status,'Max_used_connections')
        ...
        slow_queries = int(func.get_item(mysql_status,'Slow_queries')) 
        ############################# GET STATUS PERSECOND ##################################################


        threads_created_percond = int(func.get_item(mysql_status_2,'Threads_created')) - int(threads_created)
        ...
        if master_thread >= 1:
            ...
            if slave_status <> 0:
                ...
                datalist.append(int(0))
                sql="delete from mysql_replication where host=%s and port=%s;"
                param =(host,port)
                func.mysql_exec(sql,param)
        if slave_status <> 0:
            ...
            if replchannel <> '0': 
                print replchannel
                slave_info=cur.execute("show slave status for channel '%s';" %(replchannel))
            else :
                slave_info=cur.execute('show slave status;') 
            result=cur.fetchone()
        ...
        if result:
            ...
            try:
                ...
                es.index(index="logstash-lepus-mysql"+datetime.datetime.now().strftime("%Y-%m"), doc_type="lepus-status",body={"@host":host,"@port":port,"@tags":host+":"+port,"uptime":int(uptime),"version":version,"role":role,"innodb_buffer_pool_size":int(innodb_buffer_pool_size),"innodb_buffer_pool_wait_free":int(innodb_buffer_pool_wait_free),"innodb_buffer_pool_pages_total":int(innodb_buffer_pool_pages_total),"innodb_buffer_pool_pages_data":int(innodb_buffer_pool_pages_data),"innodb_buffer_pool_pages_dirty":int(innodb_buffer_pool_pages_dirty),"innodb_buffer_pool_pages_flushed":int(innodb_buffer_pool_pages_flushed),"innodb_buffer_pool_pages_free":int(innodb_buffer_pool_pages_free),"innodb_buffer_pool_pages_misc":int(innodb_buffer_pool_pages_misc),"thread_cache_hitrate":float(thread_cache_hitrate),"open_tables_usage_rate":float(open_tables_usage_rate),"opened_tables":int(open_tables),"connections_usage_rate":float(connections_usage_rate),"max_used_connections":int(max_used_connections),"thread_cache_size":int(thread_cache_size),"threads_created":int(threads_created),"threads_created_percond":int(threads_created_percond),"threads_connected":int(threads_connected),"threads_running":int(threads_running),"threads_cached":int(threads_cached),"aborted_clients":int(aborted_clients),"aborted_connects":int(aborted_connects),"connections_persecond":int(connections_persecond),"innodb_buffer_pool_read_requests_persecond":int(innodb_buffer_pool_read_requests_persecond),"innodb_buffer_pool_reads_persecond":int(innodb_buffer_pool_reads_persecond),"innodb_buffer_pool_write_requests_persecond":int(innodb_buffer_pool_write_requests_persecond),"innodb_rows_read_persecond":int(innodb_rows_read_persecond),"innodb_rows_inserted_persecond":int(innodb_rows_inserted_persecond),"innodb_rows_updated_persecond":int(innodb_rows_updated_persecond),"innodb_rows_deleted_persecond":int(innodb_rows_deleted_persecond),"innodb_row_lock_time":int(innodb_row_lock_time),"innodb_row_lock_current_waits":int(innodb_row_lock_current_waits),"innodb_row_lock_waits":int(innodb_row_lock_waits),"bytes_received_persecond":int(bytes_received_persecond),"bytes_sent_persecond":int(bytes_sent_persecond),"com_select_persecond":int(com_select_persecond),"com_insert_persecond":int(com_insert_persecond),"com_update_persecond":int(com_update_persecond),"com_delete_persecond":int(com_delete_persecond),"com_commit_persecond":int(com_commit_persecond),"com_rollback_persecond":int(com_rollback_persecond),"questions_persecond":int(questions_persecond),"queries_persecond":int(queries_persecond),"transaction_persecond":int(transaction_persecond),"open_files_usage_rate":float(open_files_usage_rate),"slow_queries":int(slow_queries),"innodb_log_waits":int(innodb_log_waits),"delay":int(0 if (result[8]=='---' or not result[8]) else result[8]),"@timestamp": datetime.datetime.now()-datetime.timedelta(hours=8)})
            except Exception,e:
                print Exception,":",e
        else :
            try:
                es = Elasticsearch([{'host': '10.1.70.220', 'port': 9200}])
                es.index(index="logstash-lepus-mysql"+datetime.datetime.now().strftime("%Y-%m"), doc_type="lepus-status",body={"@host":host,"@port":port,"@tags":host+":"+port,"uptime":int(uptime),"version":version,"role":role,"innodb_buffer_pool_size":int(innodb_buffer_pool_size),"innodb_buffer_pool_wait_free":int(innodb_buffer_pool_wait_free),"innodb_buffer_pool_pages_total":int(innodb_buffer_pool_pages_total),"innodb_buffer_pool_pages_data":int(innodb_buffer_pool_pages_data),"innodb_buffer_pool_pages_dirty":int(innodb_buffer_pool_pages_dirty),"innodb_buffer_pool_pages_flushed":int(innodb_buffer_pool_pages_flushed),"innodb_buffer_pool_pages_free":int(innodb_buffer_pool_pages_free),"innodb_buffer_pool_pages_misc":int(innodb_buffer_pool_pages_misc),"thread_cache_hitrate":float(thread_cache_hitrate),"open_tables_usage_rate":float(open_tables_usage_rate),"opened_tables":int(open_tables),"connections_usage_rate":float(connections_usage_rate),"max_used_connections":int(max_used_connections),"thread_cache_size":int(thread_cache_size),"threads_created":int(threads_created),"threads_created_percond":int(threads_created_percond),"threads_connected":int(threads_connected),"threads_running":int(threads_running),"threads_cached":int(threads_cached),"aborted_clients":int(aborted_clients),"aborted_connects":int(aborted_connects),"connections_persecond":int(connections_persecond),"innodb_buffer_pool_read_requests_persecond":int(innodb_buffer_pool_read_requests_persecond),"innodb_buffer_pool_reads_persecond":int(innodb_buffer_pool_reads_persecond),"innodb_buffer_pool_write_requests_persecond":int(innodb_buffer_pool_write_requests_persecond),"innodb_rows_read_persecond":int(innodb_rows_read_persecond),"innodb_rows_inserted_persecond":int(innodb_rows_inserted_persecond),"innodb_rows_updated_persecond":int(innodb_rows_updated_persecond),"innodb_rows_deleted_persecond":int(innodb_rows_deleted_persecond),"innodb_row_lock_time":int(innodb_row_lock_time),"innodb_row_lock_current_waits":int(innodb_row_lock_current_waits),"innodb_row_lock_waits":int(innodb_row_lock_waits),"bytes_received_persecond":int(bytes_received_persecond),"bytes_sent_persecond":int(bytes_sent_persecond),"com_select_persecond":int(com_select_persecond),"com_insert_persecond":int(com_insert_persecond),"com_update_persecond":int(com_update_persecond),"com_delete_persecond":int(com_delete_persecond),"com_commit_persecond":int(com_commit_persecond),"com_rollback_persecond":int(com_rollback_persecond),"questions_persecond":int(questions_persecond),"queries_persecond":int(queries_persecond),"transaction_persecond":int(transaction_persecond),"open_files_usage_rate":float(open_files_usage_rate),"slow_queries":int(slow_queries),"innodb_log_waits":int(innodb_log_waits),"@timestamp": datetime.datetime.now()-datetime.timedelta(hours=8)})
            except Exception,e:
                print Exception,":",e
        cur.close()
        exit
...
def main():
    ...
    func.mysql_exec("replace into mysql_status_history SELECT *,LEFT(REPLACE(REPLACE(REPLACE(create_time,'-',''),' ',''),':',''),12) from mysql_status",'')
    func.mysql_exec('delete from mysql_status where server_id not in (select id from db_servers_mysql where monitor=1);','')

    func.mysql_exec("replace into mysql_replication_history SELECT *,LEFT(REPLACE(REPLACE(REPLACE(create_time,'-',''),' ',''),':',''),12) from mysql_replication",'')
    func.mysql_exec('delete from mysql_replication where server_id not in (select id from db_servers_mysql where monitor=1);','')
    #get mysql servers list
    servers = func.mysql_query("select id,host,port,username,password,tags,repl_channel from db_servers_mysql where is_delete=0 and monitor=1 and monitor_proxy='0';")
    ...
    if servers:
         ...
         for row in servers:
             ...
             #time.sleep(1)
             p = Process(target = check_mysql, args = (host,port,username,password,server_id,tags,replchannel))
             plist.append(p)
	 for p in plist:
             p.start()
         time.sleep(10)
