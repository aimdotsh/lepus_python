...
def get_waits(conn):
    try:
        ...
        #curs.execute("select count(1) from processlist where state <> '' and user <> 'repl' and  user<>'system user' and time > 1");
        curs.execute("select count(1) from processlist where command !='Sleep' and user<>'system user' and DB not in ('information_schema','sys')");
        result = curs.fetchone()[0]
        return result
