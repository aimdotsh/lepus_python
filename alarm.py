...
'''
    table_body = ''
    #sql = "select distinct host,port,send_mail_to_list alarm_id from alarm where alarm_item='threads_waits' and send_mail=1 and level!='ok';"
    #sql = "select distinct host,port,send_mail_to_list from alarm where alarm_item='threads_running' and send_mail=1 and level!='ok';"
    sql = "select distinct a.host,a.port,b.send_mail_to_dev from alarm a join db_servers_mysql b on a.host=b.host and a.port=b.port where a.alarm_item='threads_waits' and a.send_mail=1 and a.level!='ok';"
    result=func.mysql_query(sql)
    if result <> 0:
        for line in result:
            host=line[0]
            port=line[1]
            send_mail_to_list=line[2]
            print host
            try:
                sql = "SELECT  threshold_warning_threads_waits,slow_query_time from db_servers_mysql  where host='%s' and port='%s' limit 1" %(host,port)
                cur_dbmon.execute(sql)
                thread_result = cur_dbmon.fetchone()
                #processlist_active_warning=thread_result[0]
                processlist_active_warning=1
                longquery_second=thread_result[1]
                sql = "select host,port,create_time,count(*),uuid cnt from mysql_processlist where host='%s' and port='%s' group by host,port,uuid" %(host,port)
                cur_dbmon.execute(sql)
                g_result = cur_dbmon.fetchall()
                if g_result <> 0:
                    for g_line in g_result:
                        sql = "select tags,p_user,p_host,p_db,command,time,status,info from mysql_processlist a where a.host='%s' and a.port='%s' and a.uuid='%s' order by time desc" %(g_line[0],g_line[1],g_line[4]) 
                        cur_dbmon.execute(sql)
                        result = cur_dbmon.fetchall()
                                
                        if int(g_line[3])>=int(processlist_active_warning):
                            ...
                            for line in result:
                                #print line[5]
                                #print "changjingxiu"
                                #print longquery_second
                                #print processlist_active_warning
                                if int(line[5]) >int(longquery_second):
                        ...
                        sql = "insert into mysql_processlist_history (server_id,host,port,tags,pid,p_user,p_host,p_db,command,time,status,info,uuid,create_time) select server_id,host,port,tags,pid,p_user,p_host,p_db,command,time,status,info,uuid,create_time from mysql_processlist where host='%s' and port='%s' and uuid='%s'" %(g_line[0],g_line[1],g_line[4])
                        cur_dbmon.execute(sql)
                            
                        sql = "delete from mysql_processlist where host='%s' and port='%s' and uuid='%s'" %(g_line[0],g_line[1],g_line[4])
                        cur_dbmon.execute(sql)
                    ...
                    if table_body:
                        ...
                        if send_mail_to_list:
                            ...
                            result = sendmail.send_mail(mail_to_list,"Mysql Active Processlist",mail_text)
            except Exception,e:
                print e
                print "mysql processlist got error"
    sql = "insert into mysql_processlist_history (server_id,host,port,tags,pid,p_user,p_host,p_db,command,time,status,info,uuid,create_time) select server_id,host,port,tags,pid,p_user,p_host,p_db,command,time,status,info,uuid,create_time from mysql_processlist"
    cur_dbmon.execute(sql)
    sql = "delete from mysql_processlist"
    cur_dbmon.execute(sql)
...
'''
    table_body = ''
    sql="select distinct a.host,a.port,b.send_mail_to_list from alarm a join db_servers_mongodb b on a.host=b.host and a.port=b.port where a.alarm_item in ('active_clients','current_queue') and a.send_mail=1 and a.level!='ok';"
    result=func.mysql_query(sql)
    if result <> 0:
        for line in result:
            host=line[0]
            ...
            try:
                ...
                #sql = "select host,port,create_time,count(*),uuid cnt from mysql_processlist where host='%s' and port='%s' group by host,port,uuid" %(host,port) 
                sql = "select a.host,a.port,a.CreateTime,count(*) cnt,a.uuid from mongodb_current_op a where host='%s' and port='%s' group by a.host,a.port,a.uuid"%(host,port)
                ...
                if table_body:
                    ...
                    if send_mail_to_list:
                        mail_to_list=send_mail_to_list.split(';')
                        result = sendmail.send_mail(mail_to_list,"Mongodb Active Processlist",mail_text)
            except Exception,e:
                print "analyze_mongodb_currentop got error:%s" %e
    sql="insert into mongodb_current_op_his (host,port,Op_desc,Secs_running,Op,Ns,Op_query,Op_insert,Client,Msg,Locks,WaitingForLock,LockStats,uuid,CreateTime) select host,port,Op_desc,Secs_running,Op,Ns,Op_query,Op_insert,Client,Msg,Locks,WaitingForLock,LockStats,uuid,CreateTime from mongodb_current_op"
    cur_dbmon.execute(sql)
    sql = "delete from mongodb_current_op"
    cur_dbmon.execute(sql)
...
def get_alarm_redis_status():
    ...
    if result <> 0:
        for line in result:
            ...
            tags=line[21]
            db_type=line[22]
            current_max_memory=line[23]/1024/1024
            threshold_warning_used_memory=line[24]
            threshold_critical_used_memory=line[26]
            if send_mail_to_list is None or  send_mail_to_list.strip()=='':
                send_mail_to_list = mail_to_list_common
            if send_sms_to_list is None or  send_sms_to_list.strip()=='':
                send_sms_to_list = sms_to_list_common
	    ...
	    if connect <> 1:
                ...
                if int(alarm_used_memory)==1:
                    if int(current_max_memory) >= int(threshold_critical_used_memory):
                        send_mail = func.update_send_mail_status(server_id,db_type,'used_memory',send_mail,send_mail_max_count)
                        send_sms = func.update_send_sms_status(server_id,db_type,'used_memory too large',send_sms,send_sms_max_count)
                        func.add_alarm(server_id,tags,host,port,create_time,db_type,'used_memory',current_max_memory,'critical','used_memory too large',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('memory',3,host,port,create_time,'used_memory',current_max_memory,'critical')
                    elif int(current_max_memory) >= int(threshold_warning_used_memory):
                        send_mail = func.update_send_mail_status(server_id,db_type,'used_memory',send_mail,send_mail_max_count)
                        send_sms = func.update_send_sms_status(server_id,db_type,'used_memory',send_sms,send_sms_max_count)
                        func.add_alarm(server_id,tags,host,port,create_time,db_type,'used_memory',current_max_memory,'warning','used_memory too large',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('memory',2,host,port,create_time,'used_memory',current_max_memory,'warning')
                    else:
                        func.check_if_ok(server_id,tags,host,port,create_time,db_type,'used_memory',current_max_memory,'used_memory ok',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('memory',1,host,port,create_time,'used_memory',current_max_memory,'ok')
...
def get_alarm_memcache_status():
    sql = "select a.server_id,a.connect,a.curr_connections,a.create_time,b.host,b.port,b.alarm_connections_current,b.threshold_warning_connections_current,b.threshold_critical_connections_current,b.send_mail,b.send_mail_to_list,b.send_sms,b.send_sms_to_list,b.tags,'memcache' as db_type from memcache_status a, db_servers_memcache b where a.server_id=b.id ;"
    result=func.mysql_query(sql)
    if result <> 0:
        for line in result:
            server_id=line[0]
            connect=line[1]
            curr_connections=line[2]
            create_time=line[3]
            host=line[4]
            port=line[5]
            alarm_connections_current=line[6]
            threshold_warning_connections_current=line[7]
            threshold_critical_connections_current=line[8]
            send_mail=line[9]
	    ...
	    if connect <> 1:
                send_mail = func.update_send_mail_status(server_id,db_type,'connect',send_mail,send_mail_max_count)
                send_sms = func.update_send_sms_status(server_id,db_type,'connect',send_sms,send_sms_max_count) 
                func.add_alarm(server_id,tags,host,port,create_time,db_type,'connect','down','critical','memcache server down',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_db_status('connect','3',host,port,create_time,'connect','down','critical')
                func.update_db_status('sessions','-1',host,port,'','','','')
                func.update_db_status('actives','-1',host,port,'','','','')
                func.update_db_status('waits','-1',host,port,'','','','')
                ...
                func.check_if_ok(server_id,tags,host,port,create_time,db_type,'connect','up','memcache server up',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_db_status('connect','1',host,port,create_time,'connect','up','ok')

                if int(alarm_connections_current)==1:
                    if int(curr_connections) >= int(threshold_critical_connections_current):
                        send_mail = func.update_send_mail_status(server_id,db_type,'connected_clients',send_mail,send_mail_max_count)
                        send_sms = func.update_send_sms_status(server_id,db_type,'connected_clients',send_sms,send_sms_max_count) 
                        func.add_alarm(server_id,tags,host,port,create_time,db_type,'connected_clients',curr_connections,'critical','too many connected clients',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('sessions',3,host,port,create_time,'connected_clients',curr_connections,'critical')
                    elif int(curr_connections) >= int(threshold_warning_connections_current):
                        send_mail = func.update_send_mail_status(server_id,db_type,'connected_clients',send_mail,send_mail_max_count)
                        send_sms = func.update_send_sms_status(server_id,db_type,'connected_clients',send_sms,send_sms_max_count) 
                        func.add_alarm(server_id,tags,host,port,create_time,db_type,'connected_clients',curr_connections,'warning','too many connected clients',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('sessions',2,host,port,create_time,'connected_clients',curr_connections,'warning')
                    else:
                        func.check_if_ok(server_id,tags,host,port,create_time,db_type,'connected_clients',curr_connections,'connected clients ok',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                        func.update_db_status('sessions',1,host,port,create_time,'connected_clients',curr_connections,'ok')
...
def send_alarm():
    ...
    password = func.get_config('monitor_server','passwd')
    dbname = func.get_config('monitor_server','dbname')
    conn=MySQLdb.connect(host=host,user=user,passwd=password,port=int(port),connect_timeout=3,charset='utf8')
