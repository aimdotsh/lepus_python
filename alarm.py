...
'''
    ...
    sql = "select distinct a.host,a.port,b.send_mail_to_dev from alarm a join db_servers_mysql b on a.host=b.host and a.port=b.port where a.alarm_item='threads_running' and a.send_mail=1 and a.level!='ok';"
            ...
            try:
                ...
                if g_result <> 0:
                    for g_line in g_result:
                        sql = "select tags,p_user,p_host,p_db,command,time,status,info from mysql_processlist a where a.host='%s' and a.port='%s' and a.uuid='%s' " %(g_line[0],g_line[1],g_line[4]) 
                        cur_dbmon.execute(sql)
